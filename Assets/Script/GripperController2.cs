...
public class GripperController2 : MonoBehaviour
{
    ...
    void Update()
    {
        /*DoStuff(maxPosition, minPosition, nearestBox);*/
    }

    public void DoStuff(float maxPosition, float minPosition, GameObject nearestBox)
    {
        ...
        if (nearestBox != null)
        {
            float boxPositionX = nearestBox.transform.position.x;
            ...
            {
                UpdateWhenCloseToBox(maxPosition, minPosition);
            }
            else if (hasReleased)
            {
                UpdateWhenReleased(minPosition, maxPosition);
            }
        }
    }
    ...
    void HandleGrabbedBoxMovement(float maxPosition, float minPosition)
    {
        if (transform.position.x < maxPosition && direction.Equals(Direction.Right))
        {
            HandleMovementTowardsMaxPosition();
        }
    ...
    void HandleMovementTowardsMaxPosition()
    {
        isStop = false;
